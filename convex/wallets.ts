...
function validSettlementBoundary(event: UsageEventArg): boolean {
  return (
    ...
    (event.freeTierLimit === undefined ||
      (Number.isSafeInteger(event.freeTierLimit) && event.freeTierLimit > 0)) &&
...
function validSettlementOutcome(event: UsageEventArg): boolean {
  ...
  handler: async (
    ...
    for (const event of args.events) {
      ...
      const existing = await ctx.db
        ...
        .unique();
      if (existing !== null) {
        if (await duplicateSettlementMatches(existing, wallet._id, binding)) {
      ...
      if (event.billingOutcome === "settled") {
        earningId = await ctx.db.insert("publisherEarnings", {
