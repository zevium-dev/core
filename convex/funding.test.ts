...
function usage(seed: Seed, refId: string, credits: number) {
  ...
  return {
    ...
    pricingDecision:
      credits === 0 ? ("zero_price" as const) : ("listed_price" as const),
    credits,
    billingOutcome: (credits === 0 ? "free" : "settled") as "free" | "settled",
...
describe("universal wallet funding", () => {
  ...
  it("caps one settlement transaction by total projected writes", async () => {
    ...
    // Zero-credit free calls settle no money; earnings rows are settled-only.
  ...
  it("compacts 25 distinct refundable roots without starving debit or refund", async () => {
    ...
    for (let index = 0; index < 25; index += 1) {
      paymentIds.push(await grantPayment(t, s, `twenty_five_${index}`, 1));
    }
