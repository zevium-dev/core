...
function provenancePaymentId(
  ...
  return first !== undefined &&
    slices.every((slice) => slice.paymentId === first)
    ? first
    : undefined;
}
...
async function compactLots(
  ...
  const compactedLotId = await ctx.db.insert("walletFundingLots", {
    ...
    paymentId: first.refundable ? provenancePaymentId(provenance) : undefined,
...
export async function preflightFundingAllocation(
  ...
  for (const lot of nonrefundableLots) {
    ...
    items.push({
      lot,
      grossCredits,
      provenance: splitProvenance(availableProvenance(lot), grossCredits)
        .consumed,
    });
