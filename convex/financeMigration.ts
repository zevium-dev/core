...
async function runConservationChunk(
  ...
  if (state.stage === "wallets") {
    ...
    for (const entry of page.page) {
      ...
      const payment =
        entry.paymentId === undefined
          ? null
          : await ctx.db.get(entry.paymentId);
      const usage =
    ...
    for (const entry of page.page) {
      ...
      const earning =
        entry.earningId === undefined
          ? null
          : await ctx.db.get(entry.earningId);
      const transfer =
    ...
    for (const dispatch of page.page) {
      ...
      const duplicates = await ctx.db
        ...
        .withIndex("by_transfer", (q) =>
          q.eq("transferId", dispatch.transferId),
        )
        .take(2);
      if (
        transfer === null ||
        duplicates.length !== 1 ||
        transfer.publisherOrganizationId !== dispatch.publisherOrganizationId ||
        transfer.stripeConnectedAccountId !==
          dispatch.stripeConnectedAccountId ||
    ...
    for (const allocation of page.page) {
      ...
      const homogeneousPaymentId =
        provenancePaymentIds.length === 1 ? provenancePaymentIds[0] : undefined;
    ...
    if ((state.eventOutboxMode ?? "events") === "events") {
      ...
      for (const event of page.page) {
        ...
        if (outbox === null) {
          ...
          throw new Error(
            `Stripe event ${event.stripeEventId} outbox conflicts`,
          );
        }
        state.watermark = await appendFinalWatermark(
          state.watermark,
...
export const recordLegacyRefundFacts = mutation({
  ...
  handler: async (ctx, args): Promise<{ scheduled: true }> => {
    ...
    if (
      args.refunds.length === 0 ||
      args.refunds.length > MAX_PAYMENT_SOURCES
    ) {
    ...
    for (const refund of args.refunds) {
      ...
      const requestedCredits = Math.min(
        payment.grantedCredits,
        Math.floor((payment.grantedCredits * refund.amount) / payment.amount),
      );
      const payload = {
    ...
    if (job === null || job.status === "verified") {
      throw new Error(
        "Failed finance migration is required for reconciliation",
      );
    }
    await ctx.db.patch(payment._id, {
      financeMigrationStatus: "building",
