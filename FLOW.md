...
- 🆕 Semantic search (embeddings already in schema)
...
- 🆕 Filterable call log: timestamp, API, endpoint, status, credits charged, latency; link from a charge → the exact call
...
- Per project: MCP server auto-generated from the published OpenAPI spec, billed through the same proxy/credits
- Compact tool surface (search-then-load discovery, never one-tool-per-endpoint dump — context bloat)
- Auth: same user API key
...
- 🆕 Version history: list published versions, diff between versions, rollback
...
- **Emails** ✅ (verification, invitations; templates previewable at `/$internal/email-templates-preview`) — 🆕 budget alerts, payout notices, listing-status notices
