...
const CONNECTOR_GRAMMAR = repeatedGrammar(CONNECTOR_TERM_PATTERN, 8);
const ASSERTION_FILLER_GRAMMAR = repeatedGrammar(ASSERTION_FILLER_PATTERN, 5);
const DIRECT_NEGATION_SUFFIX = new RegExp(
  `(?:^|\\s)(?<direct>(?:(?:${alternatives(AUXILIARY_WORDS)})\\s*)?(?:(?:${NEGATION_PATTERN})\\s*)+(?:(?:${NEGATION_BRIDGE_PATTERN})\\s*)*(?:(?:${alternatives(NEGATABLE_PREDICATES)})\\s*)?)\\s*$`,
  "u",
);
const DIRECT_NEGATION_PREFIX = new RegExp(
