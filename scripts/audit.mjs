...
function validateManifestPolicy(manifest, importerId) {
  ...
  if (importerId === ".") {
    ...
    for (const [name, expected] of [
      ...
      [
        "ci:pr",
        "node scripts/audit.mjs && prettier --check . && pnpm typecheck && pnpm test",
      ],
    ]) {
...
function validateWorkflowDependencyPolicy(workflow, workflowName) {
  ...
  for (const [jobName, rawJob] of Object.entries(jobs)) {
    ...
    const usesToolchain = setupNodeIndexes.length === 1;
    if (dependencyCommand && !usesToolchain) {
      throw new AuditValidationError(
        `${label} executes dependency code without pinned setup-node.`,
      );
    }
