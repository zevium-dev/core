  ...
  it("covers health, preflight, internal, discovery, MCP, mock, proxy, and 404", async () => {
    ...
    for (const [label, path, init] of cases) {
      ...
      for (const name of REQUIRED_HEADERS) {
        expect(response.headers.get(name), `${label}:${name}`).toBeTruthy();
      }
      expect(response.headers.get("content-security-policy"), label).toBe(
        GATEWAY_CSP,
      );
      expect(response.headers.get("strict-transport-security"), label).toBe(
        "max-age=31536000",
      );
      expect(response.headers.get("x-clerk-auth-reason"), label).toBeNull();
      expect(response.headers.get("x-clerk-auth-status"), label).toBeNull();
