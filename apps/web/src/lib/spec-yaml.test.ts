...
`;
  ...
  it("rejects alias bombs before post-expansion serialization", async () => {
    ...
    const result = await convertSpecInputToJson(`seed: &seed
  value: ${"x".repeat(1_000)}
expanded:
${aliases}`, workerOptions);
  ...
  it("rejects exponential expansion even below alias-count ceiling", async () => {
    const result = await convertSpecInputToJson(`a: &a [x, x, x, x, x, x, x, x, x, x]
b: &b [*a, *a, *a, *a, *a, *a, *a, *a, *a, *a]
c: &c [*b, *b, *b, *b, *b, *b, *b, *b, *b, *b]
d: [*c, *c, *c, *c, *c, *c, *c, *c, *c, *c]`, workerOptions);
    expect(result).toMatchObject({ ok: false });
  ...
  it("rejects post-expansion output beyond transport cap", async () => {
    ...
    const result = await convertSpecInputToJson(`seed: &seed ${chunk}
expanded:
${aliases}`, workerOptions);
    expect(result).toMatchObject({ ok: false });
