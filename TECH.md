...
- **Usage ingest pipe**: the wallet DO's alarm (~5s, non-empty pending queue) sends at most 25 settled calls per `POST` to `{CONVEX_SITE_URL}/ingest-usage`, an `httpAction` authenticated by a shared `x-internal-secret` header (`GATEWAY_INTERNAL_SECRET`) — no Convex deploy key on the hot path. Reservations carry an HMAC-authenticated wallet sequence/balance proof. A settlement authorized before a concurrent refund remains payable and may create consumer wallet debt; zero spendable balance still blocks every new reservation. Permanent payload/linkage rejects move to durable DO dead-letter storage while migration/transient rejects remain queued. `CONVEX_DEPLOY_KEY` remains as a fallback constructor path only, unused when the shared secret is configured. `GATEWAY_INTERNAL_SECRET` must contain at least 32 bytes.
...
- **Stripe payments**: `billing.createCheckout` creates server-priced hosted Checkout sessions. Its cancel URL carries only the local checkout-intent id; `getBillingState` resolves it under the active org before showing canceled state. Platform and Connect webhook routes verify raw-body signatures, persist each receipt and its scheduled processor in one transaction, lease processing attempts, retry with bounded backoff, and recover abandoned leases every minute. Every positive wallet source becomes a universal funding lot. Non-refundable promotion/admin inventory is consumed first, then refundable payment/restoration inventory FIFO; negative adjustments use the same fully preflighted allocator. Refund/dispute exposure removes only the affected payment's unspent inventory. Publisher clawback/restoration then runs through a durable 20-row source-specific reconciliation journal, so a million historical allocations never enter one mutation. Only Stripe `funds_withdrawn` / `funds_reinstated` events move dispute money. Stripe API version is pinned in code.
- **Staging payment-drill recovery**: run the manual `Payment Drills` workflow only from the staging environment's allowed `develop` ref. Each workflow attempt gets a distinct proof reference. The drill uses three isolated restricted test keys (Checkout/refunds, webhook administration, Connect primitives), an exact canonical webhook endpoint id, and an exact settlement account id. Its paid-call evidence follows exact `x-zevium-request-id` values through `settle:{requestId}` wallet entries and usage rows; contiguous ledger sequences reject unrelated wallet writes. Its intentional failure is a disposable `charge.refunded` endpoint tagged with `purpose=zevium_payment_drill_canary` and the attempt reference; it never edits the canonical endpoint. The proof requires exactly one canonical Convex receipt, exactly one pending Stripe delivery, an exhaustive two-endpoint v1 topology, and zero competing v2 snapshot destinations before attributing the pending delivery to the canary. Workflow concurrency serializes canary reconciliation. A failed job must still run `PAYMENT_DRILL_PHASE=cleanup e2e/04-payment-drill.sh`, which removes only that attempt's tagged canary, expires an unpaid Checkout or refunds the exact remaining paid amount, then waits for the matching Convex receipt, payment projection, wallet balance, and immutable reversal journal to conserve. Provider cleanup closes the attempt's temporary account, reverses its test payout and transfers, and refunds both source charges. After runner loss, locate the test Checkout by `checkoutIntentId`, `clerkOrgId`, and workflow time; refund only its remaining charge, retry its exact event to the configured canonical endpoint, verify `paymentEvents.deliveries`, `payments`, `walletFundingReversals`, and `wallets`, then run `cleanup-webhook-canary` with the failed attempt reference. Run `provider-cleanup` with that same reference for a failed Connect proof. Rerun the workflow from `develop`, rotate any proof key exposed to logs, and never accept or close payment proof work without one green manual run and its sanitized evidence artifacts.
- **Finance schema rollout**: finance-v2 legacy fields stay optional in this deploy and every reader uses conservative zero/derived defaults. `financeMigration.start` is admin-only, versioned, resumable, and processes at most 25 rows per transaction. It replays ordered wallet ledger entries into universal sources/allocations/reversal journals, normalizes source clawbacks, rebuilds publisher aggregates from immutable settlement/earning/transfer facts, reconstructs transfer reversal totals and secure correlation, then writes per-scope conservation audits before status becomes `verified`. No validator is tightened in this phase. Required-validator contract is a separate later deploy: first require verified `finance-v2-universal-funding` audit with zero migration errors; then make `wallets.debtCredits`, payment reversal aggregates, publisher balance aggregates, clawback allocation/restoration/state fields, and transfer reversal/correlation fields required; remove conservative `?? 0` reads and legacy `paymentFundingLots` / `paymentFundingAllocations` only after that schema deploy validates production.
- **Connect settlement**: publisher usage creates risk-held earning rows denominated in accounting atoms (`10,000 atoms = 1 credit`), making every per-call 95/5 split exact even for a one-credit call. Mature earnings post in indexed 25-row chunks into an append-only publisher settlement ledger plus materialized available / allocated / paid buckets and all-row pending-risk / reversed / failed aggregates; recent-row pagination is display-only. Refund/dispute clawbacks can make available balance negative after already-paid earnings, so future earnings repay debt before payout. Transfers require the product's $10 minimum, allocate only whole Stripe cents, and leave all sub-cent atoms in canonical available balance. Transfer creation and webhook/crash recovery validate amount, currency, destination connected account, platform account, 256-bit server nonce, and HMAC metadata; cumulative reversals return the exact allocation without duplicating or dropping remainder. `STRIPE_PLATFORM_ACCOUNT_ID` and a 32-byte `STRIPE_TRANSFER_CORRELATION_SECRET` are required before transfer creation or legacy transfer migration.
