...
- **Gateway CORS**: `/gateway`, `/mock`, `/discovery`, `/mcp` all allow wildcard origin. `/gateway` and `/mcp` use bearer keys rather than cookies; `/mock` and `/discovery` are deliberate public read/synthesis surfaces. None use cookie authentication, so wildcard origin does not grant browser ambient authority
...
- **Publisher upstream credentials**: project Settings writes AES-GCM encrypted header secrets into server-only `upstreamCredentials` rows. Convex requires `UPSTREAM_CREDENTIAL_ENCRYPTION_KEYS` as JSON `{ current, keys }`; retain prior key ids during re-encryption rotation. `GATEWAY_INTERNAL_SECRET` authenticates transport only and is never encryption material. The transitional `migrateLegacyPlaintext` internal mutation encrypts legacy rows before the schema tightens to required ciphertext fields. Gateway resolves published spec + secrets through shared-secret-authenticated `GET /gateway-spec`, caches the result for 30s, strips consumer `Authorization`/`x-api-key`, then injects publisher headers before upstream fetch. Public catalogue, discovery, mock, and OpenAPI payloads never contain secret values.
  - Rollout command: `pnpm exec convex run admin:migrateSecurityRollout --identity '{"subject":"<admin-user-id>","org_id":"<admin-org-id>","org_role":"org:admin"}'`. Re-run until `remainingPlaintext`, `remainingUnencrypted`, and `remainingMissingHandles` are all exactly zero and `handles.blocked` is zero; only then make encryption/public-handle validators required and remove transitional fields. Unsafe legacy handles remain missing for private owner remediation instead of being copied into public routes.
- **Public publisher-copy boundary**: organization name/Clerk slug/public handle, project metadata, version/deprecation copy, and every OpenAPI key/string are checked independently on writes and again at every public read; document/surface composition checks also reject framework/assurance claims split across fields without allowing cross-field negation. Bidi controls, common confusables, one-edit protected-framework mutations, unsupported generic assurance badges and ambiguous bounded-nearby framework/assurance copy fail closed; only bounded direct-negative, pending, and evidence-analysis grammar is exempt. Malformed JSON and unsafe legacy rows fail closed from catalogue, semantic search, direct spec lookup, gateway, mock, discovery and MCP. Owner/admin private reads remain available for remediation. MCP puts publisher fields under an explicit untrusted-data object and never interpolates them into trusted usage instructions.
- **URL spec import**: TanStack POST server function requires strict same-origin request, Clerk user plus active org/member claims, and an authenticated Convex rate lease before network access. Each HTTPS redirect hop is checked with Cloudflare-supported [`resolve4`/`resolve6`](https://developers.cloudflare.com/workers/runtime-apis/nodejs/dns/) calls and rejects any non-public answer; `global_fetch_strictly_public` adds runtime egress defense. Approved JSON/YAML/text MIME, 2 MiB, five redirects and 10-second total timeout are enforced.
- **Static claim scanner**: Git-tracked inputs are combined with web manifest-derived output and gateway Wrangler dry-run/metafile output. Generated provenance is retained through deduplication, so no source-only path/digest exception can transfer to a deploy input. No global test/generated/dist/extension bypass exists. Files are byte-bounded; unknown binary and every recognized opaque binary, compressed stream, archive, PDF, font, database, or media container scan input are rejected because no exact decoder exists. Brotli has no magic number, so conventional `.br` names and renamed non-empty streams found by a bounded decompression probe fail closed; oversized probe output also fails closed. A future decoder may allow a format only after enforcing compressed/input bytes, expanded bytes, entry count, nesting depth, normalized in-root member paths, and complete decoded-text scanning. Retired, unreferenced non-deploy media left in the tree are excluded only by exact path and whole-file SHA-256; the web build removes old public raster copies, and any new tracked reference, changed bytes, rename, explicit scan, or generated copy fails closed. Relative-path normalization converts only the host platform separator, so POSIX backslash filenames cannot alias an exception. In-repo symlink checks and exact-path SHA-256 hostile-test fixtures also fail closed. Web builds delete and the scanner rejects generated `.dev.vars` so local secrets cannot enter preview artifacts.
