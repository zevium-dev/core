...
- **Static claim scanner**: Git-tracked inputs are combined with web manifest-derived output and gateway Wrangler dry-run/metafile output. No global test/generated/dist/extension bypass exists. Files are byte-bounded; unknown binary and every recognized opaque binary, compressed stream, archive, PDF, font, database, or media container scan input are rejected because no exact decoder exists. A future decoder may allow a format only after enforcing compressed/input bytes, expanded bytes, entry count, nesting depth, normalized in-root member paths, and complete decoded-text scanning. Retired, unreferenced non-deploy media left in the tree are excluded only by exact path and whole-file SHA-256; the web build removes old public raster copies, and any new tracked reference, changed bytes, rename, explicit scan, or generated copy fails closed. In-repo symlink checks and exact-path SHA-256 hostile-test fixtures also fail closed. Web builds delete and the scanner rejects generated `.dev.vars` so local secrets cannot enter preview artifacts.
