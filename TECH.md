...
- **Preview verification**: every trusted PR deploys isolated Convex, gateway, and web previews and runs the required anonymous smoke plus gateway health check. The authenticated publisher/consumer journey is intentionally opt-in because it is long and stateful: add the `full-e2e` label to a PR, or run **Pull Request Preview** manually on the PR head with its PR number. The full job consumes the preview URLs emitted by the deploy-and-smoke job; closed PRs invoke the separate preview cleanup workflow.
