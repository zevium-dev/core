...
- Convex ledger is authoritative; DO holds a monotonic balance/sequence checkpoint plus active reservations and pending settlements. DO batches stable settlement refs to Convex; Convex returns per-ref `applied`/`already_applied`/`rejected` outcomes and an authoritative checkpoint. Applied and exact-replay refs are acknowledged. Terminal rejections are dead-lettered and rebase even from a same-sequence checkpoint, restoring the exact local debit; transient/migration rejections remain queued. Lost acknowledgements, partial rejection, and DO eviction therefore converge without ghost debits.
