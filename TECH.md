...
- `qualityProbeTargets`, immutable `qualityProbeResults`, denormalized `qualitySnapshots`, and `qualityIncidents` (credential-free public upstream monitoring; DNS-pinned SSRF guard)
- `listingSubscriptions`, verified `reviews` + append-only edit/moderation/response audit tables, and server-owned `reviewAggregates`
...
- **Quality probes + publish reachability**: publish readiness and scheduled probes send credential-free `HEAD` requests directly to the spec's HTTPS upstream. Resolution rejects any private, reserved, mixed public/private, or non-global answer; sockets pin a vetted address so DNS rebinding cannot redirect the connection. Redirects are re-resolved and re-validated, port is fixed to 443, total time/redirects/batch size are bounded, and this path never touches gateway usage, credits, or earnings. Results and incidents are scoped to the exact immutable spec version; delayed jobs cannot overwrite newer state. A rolling 24-sample snapshot exposes availability (any HTTP response / attempts), HTTP success (2xx/3xx / responses), response p50, and spec freshness only after three samples. A 4xx proves availability without claiming HTTP success or opening an outage; transport failures and 5xx responses drive incident state.
- **Verified reviews**: eligibility comes only from gateway-settled `usageEvents` (`settle:*`) for the active consumer org and listing. One durable review row per consumer org/listing supports idempotent audited edits/withdraw/reactivation; only active, non-hidden reviews affect transactional server-owned aggregates. Public results expose an anonymous "Verified consumer" label, never the buyer organization id/name. Publisher responses can be created or edited but not deleted. Admin hide/restore actions require a reason, store the platform actor, and converge safely on duplicate delivery.
