...
- **Usage ingest pipe**: the wallet DO's alarm (~5s, non-empty pending queue) batches settled usage and `POST`s it to `{CONVEX_SITE_URL}/ingest-usage`, an `httpAction` authenticated by a shared `x-internal-secret` header (`GATEWAY_INTERNAL_SECRET`) — no Convex deploy key on the hot path. `CONVEX_DEPLOY_KEY` remains as a fallback constructor path only, unused when the shared secret is configured. Release automation uses a separate `RELEASE_PROBE_SECRET` on request-scoped `/release-probe-accounting`; response proves exact usage event, wallet settlement entry, and publisher earning split without consumer identity, key, or balance
...
- **Production releases**: exact git SHA is immutable release identity exposed by web metadata and gateway `/health`. One concurrency group serializes production mutations. Same SHA must pass staging cross-service contracts plus authenticated E2E that proves a positive-cost call reached exact usage/ledger/publisher settlement before production approval. After approval, current `develop` and active web/gateway identities are re-resolved before paid baseline and mutation. Cloudflare versions upload before traffic changes and retain explicit rollback pointers. Production candidates enter at 0% traffic and are addressed through version overrides; web verification fetches stamped HTML plus its hashed assets before promotion, avoiding frontend version skew. Convex deploys backward-compatible expansions first; old authenticated paid contract is re-proved against expanded control plane before gateway then web move to 100%. Later destructive contracts use separate manual approval, leave durable verified commit-status marker required by every descendant normal release, and roll forward on failure because Convex offers no cross-vendor atomic rollback. Gateway Durable Object lifecycle config/migrations are excluded from generic Versions/rollback path and require isolated expand/contract approval plus atomic deploy; failure is roll-forward. Sanitized evidence records state without secrets or response bodies. Operator state machine and recovery steps live in [docs/production-deploy.md](docs/production-deploy.md).
