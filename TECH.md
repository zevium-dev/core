...
- Convex ledger is authoritative; DO holds a monotonic balance/sequence checkpoint plus active reservations and pending settlements. Every reservation snapshots the immutable published spec id/version, operation id, listed/free-tier decision, stable key family, and complete pre-request monthly-budget counters. DO batches stable settlement refs to Convex; Convex hashes that full identity and returns per-ref `applied`/`already_applied`/`rejected` outcomes plus an authoritative checkpoint. Applied and exact-replay refs are acknowledged. Terminal rejections are dead-lettered and rebase even from a same-sequence checkpoint, restoring the exact local debit; transient/migration rejections remain queued. Lost acknowledgements, partial rejection, and DO eviction therefore converge without ghost debits or repricing.
...
- `wallets` (ledger: grants, settlements, refund/dispute reversals and restorations — append-only entries + materialized balance; reservations live only in the DO)
...
- `organizationPayments`, `checkoutIntents`, `payments`, `paymentEvents`, `stripeEventOutbox` (Stripe customer/Connect projection, hosted Checkout correlation, durable leased webhook inbox/outbox)
- `publisherEarnings`, `publisherTransfers`, `publisherTransferDispatches`, `connectedPayouts` (risk-held 95/5 earnings, transfer dispatch journal/provider proof, bank-payout projection)
...
- **Usage ingest pipe**: the wallet DO's alarm (~5s, non-empty pending queue) sends at most 100 settled calls per `POST` to `{CONVEX_SITE_URL}/ingest-usage`, an `httpAction` authenticated by a shared `x-internal-secret` header (`GATEWAY_INTERNAL_SECRET`) — no Convex deploy key on the hot path. Convex rechecks canonical balance, complete universal-funding inventory, immutable spec-version ownership, and the SHA-256 settlement fingerprint at settlement time, then admits only the prefix fitting the shared total-write budget; retryable remainder stays queued and drains through resumable chunks. A stale reservation after any adverse ledger change is terminally denied; consumer debt and source-less allocations do not exist. Permanent payload/linkage/insufficient-balance rejects move to durable DO dead-letter storage and restore the exact edge checkpoint, while migration/transient rejects remain queued. `CONVEX_DEPLOY_KEY` remains as a fallback constructor path only, unused when the shared secret is configured. `GATEWAY_INTERNAL_SECRET` must contain at least 32 bytes.
...
- **Stripe payments**: `billing.createCheckout` is admin/owner-only and creates server-priced hosted Checkout sessions. Its cancel URL carries only the local checkout-intent id; `getBillingState` resolves it under the active org before showing canceled state. Platform and Connect webhook routes verify raw-body signatures, then atomically persist receipt, outbox row, and scheduled processor. Random lease tokens reject stale completions; bounded attempt cycles preserve cumulative attempt evidence, cron recovers abandoned leases, and exhaustion terminates in `provider_reconciliation_required` for money events or explicit `dead_letter` for non-money events. Admin resume starts a new attempt cycle without erasing history. Every positive wallet source becomes a universal funding lot. Non-refundable promotion/admin inventory is consumed first, then refundable payment/restoration inventory FIFO; negative adjustments use the same fully preflighted allocator. Pairwise compaction spans compatible roots (including different refundable payments), preserves exact source/payment slices and immutable root-to-derived lineage, and keeps active lot/write fan-out bounded, so 25+ roots cannot permanently starve a debit. Refund/dispute exposure removes only the affected payment slices from mixed inventory. Pending and `requires_action` refunds reserve exposure; failed/canceled transitions restore publisher exposure first and then mint an exact payment-bound restoration lot for wallet inventory. Publisher clawback/restoration runs through a durable eight-row source-specific reconciliation journal, so a million historical allocations never enter one mutation. Only Stripe `funds_withdrawn` / `funds_reinstated` events move dispute money. Stripe API version is pinned in code.
- **Finance schema rollout**: finance-v2 legacy fields stay optional in this expansion deploy, but optional money reads and writes fail closed unless the versioned global migration job and each touched wallet/payment/publisher scope are verified. `financeMigration.start` is admin-only and resumable. Its singleton insertion captures a stable global software-fence generation and conflicts with concurrent gated scope deletion/creation. It fences each scope as `building`, replays at most one money-bearing history row per transaction, records sequence watermarks, then independently enumerates every money parent and child table to detect orphan/cross-scope rows while recomputing ledger, lot, allocation, reversal, exposure, publisher, transfer, exact 95/5, and cross-table conservation. A deterministic hash over that fenced snapshot plus all table counts becomes the final watermark in the same transaction that rechecks unresolved webhook/reconciliation/transfer ranges and changes the job to `verified`. Failed chunks persist a failed audit and keep the global fence closed. Legacy refund totals without exact provider ids/amounts/statuses become `provider_reconciliation_required`; migration never fabricates succeeded refunds. Legacy transfers also remain `provider_repair_required`: repair retrieves a persisted Stripe id or requires one exact result from two complete paginated listing passes, never issues `transfers.create`, adds v2 fingerprint metadata under a distinct repair key, retrieves Stripe's snapshot, resets final conservation, and records `provider_verified` only after exact amount/currency/destination/platform/nonce/HMAC/fingerprint/reversal agreement. No validator is tightened in this phase. Required-validator contraction is a separate later deploy, gated on the verified `finance-v2-universal-funding-v2` final watermark, zero unfinished jobs, provider proof for every legacy transfer, and zero missing optional finance fields; it then deletes legacy consumer-debt and `paymentFundingLots` / `paymentFundingAllocations` compatibility fields/tables.
- **Connect settlement**: publisher usage creates risk-held earning rows denominated in accounting atoms (`10,000 atoms = 1 credit`), making every per-call 95/5 split exact even for a one-credit call. Mature earnings post in indexed 25-row chunks into an append-only publisher settlement ledger plus materialized available / allocated / paid buckets and all-row pending-risk / reversed / failed aggregates; recent-row pagination is display-only. Refund/dispute clawbacks can make publisher available balance negative after already-paid earnings, so future earnings repay that publisher liability before payout; consumer wallets never carry debt. Transfers require the product's $10 minimum, allocate only whole Stripe cents, and leave all sub-cent atoms in canonical available balance. Transfer allocation, immutable create request fingerprint, and dispatch row commit before first provider attempt. A leased attempt persists first-attempt time and a conservative 23-hour retry window; response loss reconciles through two complete, stable, paginated provider listings. Within that safe window an exact no-match may retry with the original idempotency key; outside it, or on truncated/inconsistent/conflicting/multiple matches, no create occurs and explicit provider reconciliation is required. Webhook projection and operator reconciliation validate amount, currency, destination connected account, platform account, 256-bit server nonce, HMAC, request fingerprint, and cumulative reversal snapshot. `STRIPE_PLATFORM_ACCOUNT_ID` and a 32-byte `STRIPE_TRANSFER_CORRELATION_SECRET` are required before transfer creation or legacy transfer repair.
...
- **Payouts**: Stripe Connect onboarding replaces free-form payout destinations. Earning-row status describes risk/clawback state; canonical money movement lives in publisher settlement entries and balance buckets. `/admin/payouts` exposes transfer-dispatch state, safe retries, list-only provider reconciliation, and legacy metadata proof while Stripe payout events project bank-delivery state.
