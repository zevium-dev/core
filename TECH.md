...
- `gatewayControlOutbox`, `gatewayOrgControls` (durable monotonic registry projection), `catalogueListings` + `catalogueStats` (bounded public read model)
...
- **Gateway registry projection**: control-plane mutations transactionally append immutable, per-entity monotonic events to `gatewayControlOutbox`; an action signs `timestamp.nonce.rawBody` with HMAC-SHA256 and retries with bounded backoff until gateway returns `applied`, `duplicate`, or `stale` for the exact operation/revision. Organization archive is one global kill switch, independent of paginated key cleanup. `GET /gateway-control-manifest` provides a shared-secret-authenticated, version-pinned, paginated bootstrap; any revision change forces restart instead of mixed snapshots.
