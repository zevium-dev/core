...
- **Production releases**: exact git SHA is immutable release identity exposed by web metadata and gateway `/health`. Shared concurrency intentionally coalesces superseded normal SHAs into current `develop`; generic preflight enumerates every protected contract/lifecycle commit since active production, so protected work cannot be silently skipped. Contract and DO lanes emit GitHub-signed custom attestations bound to exact workflow/run/head, reviewed deployment environment, active/protected bases, phase, and canonical digest; raw commit statuses are not trusted. Every approved job runs GitHub stale/CI guard before checkout/setup, and every provider mutation rechecks identity plus exact active provider config in same shell block. Same SHA passes staging paid accounting and exact request-id/challenge E2E before production approval. Production persists recovery intent before mutation, uploads immutable Worker candidates, deploys compatible Convex expansion, verifies 0%-traffic candidates through version overrides, then promotes gateway and web. Signals record ambiguity only; separately protected recovery re-resolves captured bounded versions and proves paid accounting. All paid probes resolve one exact active Clerk key at runtime from Clerk secret key plus org slug/member/key constraints, mask immediately, and never persist it. Generic release rejects all DO lifecycle diffs before staging; dedicated lifecycle uses atomic `wrangler deploy`, never rolls back shared DO state, and fails closed when provider metadata cannot prove declarative tombstones/transfers. See [production deploy protocol](docs/production-deploy.md).
