...
- **Stripe payments**: `billing.createCheckout` creates server-priced hosted Checkout sessions. Platform and Connect webhook routes verify raw-body signatures, persist each receipt and its scheduled processor in one transaction, lease processing attempts, retry with bounded backoff, and recover abandoned leases every minute. Payment grants retain FIFO funding-lot attribution: refund/dispute exposure removes that payment's remaining wallet value first, then claws only publisher earnings funded by consumed credits; restoration unwinds the same allocation deterministically. Refund and per-dispute projections update consumer wallet, payment state, and publisher clawbacks atomically. Dispute lifecycle events never imply money movement: only Stripe `funds_withdrawn` / `funds_reinstated` events reverse / restore credits. Stripe API version is pinned in code.
- **Connect settlement**: publisher usage creates risk-held earning rows denominated in accounting atoms (`10,000 atoms = 1 credit`), making every per-call 95/5 split exact even for a one-credit call. Mature earnings post into an append-only publisher settlement ledger plus materialized available / allocated / paid buckets and all-row pending-risk / reversed / failed aggregates; recent-row pagination is display-only. Refund/dispute clawbacks can make available balance negative after already-paid earnings, so future earnings repay debt before payout. Transfers require the product's $10 minimum, allocate only whole Stripe cents, and leave all sub-cent atoms in canonical available balance; transfer reversals reconcile Stripe's cumulative reversed amount and return the exact allocation without duplicating or dropping remainder.
