...
- Server-side spec import fetches a user-selected HTTPS URL after DNS/private-IP
  and redirect checks. The destination host receives the request URL (including
  its path/query), source network metadata, and a generic OpenAPI `Accept`
  header. Zevium transiently buffers up to 2 MiB of returned spec content and
  returns it to the authenticated browser editor; import code does not write it
  to an application table until the user separately saves the draft.
...
| Spec import (transient)                 | User-selected HTTPS URL including path/query; DNS resolution, source IP/network and request metadata; redirect targets; response content type and up to 2 MiB of returned OpenAPI text | Fetch a spec server-side for the authenticated editor         | Cloudflare web runtime and each user-selected external host; returned text transits to browser memory                          | SSRF guards re-check redirects and reject private addresses; 10-second wall timeout and 2 MiB cap. No intentional app-table write occurs during fetch, but platform/destination logs and destination retention are unverified; later draft save is a separate Convex flow |
...
| User-selected spec-import host | Import URL path/query, source network/request metadata, redirect requests, and transient returned OpenAPI content up to 2 MiB | User warning and purpose limitation; destination terms/privacy responsibility; egress/log review; SSRF tests; content-handling and deletion evidence                                          | Engineering Lead + Legal/Privacy Counsel | Partial—HTTPS, DNS/private-IP, redirect, timeout and size controls exist; arbitrary destination governance, platform logging and user notice are absent                                                                                                                                                                               |
...
`pnpm check:compliance-claims` runs normalization/adversarial tests, then safely
enumerates every Git-tracked file regardless of root. Known text/code/config
extensions are scanned even if malformed with NUL bytes; unknown NUL-free files
also enter scope. Only this candid posture, claim-scanner implementation, test
fixtures, known binary formats, generated output and dependency directories are
excluded. New YAML, XML, Astro, extensionless config and repository roots enter
scope without a target-list edit. Matching normalizes compatibility forms, common
Unicode homoglyphs, zero-width/format characters, punctuation and separators.

Publisher-controlled organization/project name, description, tags, OpenAPI
titles/descriptions/summaries/tags and public deprecation copy are rejected at
write/save/publish/make-public boundaries; no human-review override state exists.
Catalogue, semantic search, discovery and MCP re-check and omit legacy/bypassed
unsafe rows. Tests cover those generated-copy publication paths. Negative and
narrow technical statements remain allowed. This is still a heuristic tripwire,
not legal review or image OCR: raster text, third-party pages and novel wording
need assigned human review. A pass proves only that current patterns and
boundaries found no prohibited copy.
