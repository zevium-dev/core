...
function exportedValues(path, sources, cache, stack = new Set()) {
  ...
  if (source === undefined)
    throw new Error(`Missing Convex dependency source: ${path}`);
  ...
  for (const statement of source.statements) {
    ...
    if (targetPath === undefined) {
      throw new Error(
        `Unresolved Convex dependency: ${path} -> ${statement.moduleSpecifier.text}`,
      );
    }
    const target = exportedValues(
      targetPath,
      sources,
      cache,
      new Set([...stack, path]),
...
function httpInventory(_path, source, constants) {
  const routes = {};
