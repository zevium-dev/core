...
export const Route = createFileRoute("/app/organizations/$organizationSlug")({
  ...
  loader: async ({ context: { queryClient, trpc }, params: { organizationSlug } }) => {
    await queryClient.ensureQueryData(trpc.organization.get.queryOptions({ organizationSlug }));
  },
});
  ...
  if (!org) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        <Typography variant="large">Organization not found</Typography>
      </div>
    );
  }
    ...
    <>
      <PageHeaderContent>
        <div className="flex items-center gap-3">
          ...
          <Typography variant="large">{org.name}</Typography>
        </div>
      </PageHeaderContent>
